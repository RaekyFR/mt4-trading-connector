 ...
 * Reçoit les signaux de TradingView avec support entry_price/current_price
 */
router.post('/tradingview', async (req, res) => {
  try {
    ...
    if (direction) {
      normalizedAction = direction.toLowerCase();
    } else if (action) {
    ...
    // Validation des nouveaux champs prix
    const entryPrice = req.body.entry_price;
    ...
    // Mapper les champs TradingView vers format interne
    const normalizedSignal = {
      ...
      stopLoss: req.body.stop_loss, // Mapper stop_loss -> stopLoss
      takeProfit: req.body.take_profit, // Mapper take_profit -> takeProfit
      lot: req.body.lot,
...
/**
 * GET /webhook/test
 * Page de test mise à jour avec support entry_price/current_price
 */
router.get('/test', (req, res) => {
  res.send(`
    ...
    <html>
    <head>
      <title>Test Webhook TradingView - Market/Limit Orders</title>
      <style>
    ...
    <body>
      <h1>Test Webhook TradingView - Market/Limit Orders</h1>
      ...
      <form id="webhookForm">
        ...
        <div class="form-section">
          ...
          <div class="form-group">
            <label>Lot (calculé auto si vide):</label>
            <input type="number" name="lot" step="0.01" placeholder="Taille de position">
          </div>
        </div>
      ...
      <script>
        // Presets pour différents types d'ordres
        const presets = {
          market: {
            signal: 'FVG_ENTRY',
            direction: 'BUY',
        ...
        document.getElementById('webhookForm').addEventListener('submit', async (e) => {
          ...
          try {
            ...
            if (response.ok) {
              responseDiv.innerHTML = '<div class="success">✅ Signal accepté:<br><pre>' + JSON.stringify(result, null, 2) + '</pre></div>';
            } else {
